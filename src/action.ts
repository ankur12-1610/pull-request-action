...
async function run() {
    ...
    const response: AxiosResponse =  await axios.get(`https://api.giphy.com/v1/gifs/random?api_key=pu4fhiXtsJYe64zjVY0Y9mIwMSqfs4BZ&tag=thanks`)
    const gifUrl: string = response.data.data.images.original.url

    const gif = `![thanks](${gifUrl})`
