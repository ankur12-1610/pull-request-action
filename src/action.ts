...
async function run() {
    ...
    const firstPullAuthor = firstPull.data[0].user.login  //first contributor to PR
    if (firstPullAuthor === author) {
      await octokit.rest.issues.createComment({
        ...context.repo,
        issue_number: pull_request.number,
        body: `:tada: Thanks for contributing to the project! :tada:`
      })
    }
